...
__export(src_exports, {
  ...
  AptosConnectWalletPlugin: () => AptosConnectWalletPlugin,
...
var _AptosConnectWallet = class _AptosConnectWallet {
  constructor({ network = import_ts_sdk2.Network.MAINNET, ...clientConfig }) {
    // endregion
    // region AptosWallet
    ...
    this.url = walletUrl;
    this.chains = import_wallet_standard2.APTOS_CHAINS;
    this.client = new import_dapp_sdk.ACDappClient(clientConfig);
    if (!import_ts_sdk2.NetworkToNodeAPI[network]) {
      throw new Error("Network not supported");
    }
    const aptosConfig = new import_ts_sdk2.AptosConfig({ network });
    this.aptosClient = new import_ts_sdk2.Aptos(aptosConfig);
  }
  static get connectedAccount() {
    ...
    try {
      ...
      return serialized ? new import_wallet_standard2.AccountInfo({
        address: import_ts_sdk2.AccountAddress.from(address),
  ...
  get features() {
    return {
      [import_wallet_standard2.AptosConnectNamespace]: {
        connect: this.connect.bind(this),
        version: "1.0.0"
      },
      [import_wallet_standard2.AptosDisconnectNamespace]: {
        disconnect: this.disconnect.bind(this),
        version: "1.0.0"
      },
      [import_wallet_standard2.AptosGetAccountNamespace]: {
        account: this.getAccount.bind(this),
        version: "1.0.0"
      },
      [import_wallet_standard2.AptosGetNetworkNamespace]: {
        network: this.getNetwork.bind(this),
        version: "1.0.0"
      },
      [import_wallet_standard2.AptosOnAccountChangeNamespace]: {
        onAccountChange: this.onAccountChange.bind(this),
        version: "1.0.0"
      },
      [import_wallet_standard2.AptosOnNetworkChangeNamespace]: {
        onNetworkChange: this.onNetworkChange.bind(this),
        version: "1.0.0"
      },
      [import_wallet_standard2.AptosSignAndSubmitTransactionNamespace]: {
        signAndSubmitTransaction: this.signAndSubmitTransaction.bind(this),
        version: "1.1.0"
      },
      [import_wallet_standard2.AptosSignMessageNamespace]: {
        signMessage: this.signMessage.bind(this),
        version: "1.0.0"
      },
      [import_wallet_standard2.AptosSignTransactionNamespace]: {
  ...
  async connect() {
    ...
    if (connectedAccount !== void 0) {
      return { args: connectedAccount, status: import_wallet_standard2.UserResponseStatus.APPROVED };
    }
  ...
  async getAccount() {
    const { connectedAccount } = _AptosConnectWallet;
    if (!connectedAccount) {
      throw new import_wallet_standard2.AptosWalletError(import_wallet_standard2.AptosWalletErrorCode.Unauthorized);
    }
  ...
  async getNetwork() {
    ...
    const url = import_ts_sdk2.NetworkToNodeAPI[network];
    return {
      chainId,
      name: network,
      url
...
var import_dapp_sdk2 = require("@identity-connect/dapp-sdk");
...
function convertV1toV2(src, dst) {
  ...
  const deserializerV2 = new import_ts_sdk3.Deserializer(serializedBytes);
...
function customAccountToStandardAccount2({ address, name, publicKey }) {
  if (publicKey instanceof import_ts_sdk4.MultiEd25519PublicKey || publicKey instanceof import_ts_sdk4.MultiKey) {
    throw new Error("Unsupported public key type");
  }
  return {
...
function accountAuthenticatorToTransactionAuthenticator(accountAuthenticator) {
  if (accountAuthenticator instanceof import_ts_sdk4.AccountAuthenticatorEd25519) {
    return new import_ts_sdk4.TransactionAuthenticatorEd25519(accountAuthenticator.public_key, accountAuthenticator.signature);
  }
  if (accountAuthenticator instanceof import_ts_sdk4.AccountAuthenticatorSingleKey) {
    if (accountAuthenticator.isEd25519()) {
      return new import_ts_sdk4.TransactionAuthenticatorEd25519(
        accountAuthenticator.public_key.publicKey,
        accountAuthenticator.signature.signature
      );
    }
    return new import_ts_sdk4.TransactionAuthenticatorSingleSender(accountAuthenticator);
  }
...
function unwrapUserResponse(response) {
  if (response.status === "dismissed") {
    throw new import_wallet_standard3.AptosWalletError(0, "Rejected");
  }
  return response.args;
}
var _AptosConnectWalletPlugin = class _AptosConnectWalletPlugin {
  constructor({ network = import_ts_sdk4.Network.MAINNET, ...clientConfig }) {
    ...
    this.url = walletUrl;
    this.icon = walletIcon;
    this.client = new import_dapp_sdk2.ACDappClient(clientConfig);
    if (!import_ts_sdk4.NetworkToNodeAPI[network]) {
      throw new Error("Network not supported");
    }
    const aptosConfig = new import_ts_sdk4.AptosConfig({ network });
    this.aptosClient = new import_ts_sdk4.Aptos(aptosConfig);
  }
  static get connectedAccount() {
  ...
  async signAndSubmitTransaction(payloadV1InputOrGenerateTxnInput, optionsV1) {
    ...
    const response = await this.client.signAndSubmitTransaction({
      signerAddress: import_ts_sdk4.AccountAddress.from(connectedAccount.address),
      ...options,
      payload
    });
  ...
  async signAndSubmitBCSTransaction(payloadV1, options) {
    ...
    const payload = convertV1toV2(payloadV1, import_ts_sdk4.TransactionPayload);
    const response = await this.client.signAndSubmitTransaction({
  ...
  async signTransaction(transactionOrPayload, optionsOrAsFeePayer) {
    ...
    if (!(transactionOrPayload instanceof import_aptos2.TxnBuilderTypes.TransactionPayload)) {
      throw new Error("Not supported");
    }
    const payload = convertV1toV2(transactionOrPayload, import_ts_sdk4.TransactionPayload);
    const options = optionsOrAsFeePayer;
    const sender = options?.sender ? {
      address: import_ts_sdk4.AccountAddress.from(options.sender)
    } : void 0;
    const response = await this.client.signTransaction({
    ...
    const signedTransaction = new import_ts_sdk4.SignedTransaction(rawTransaction, txnAuthenticator);
  ...
  async network() {
    const { network } = this.aptosClient.config;
    const chainId = await this.aptosClient.getChainId();
    const url = import_ts_sdk4.NetworkToNodeAPI[network];
    return {
