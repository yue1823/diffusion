...
// import {  TokensMapping, NETWORKS_MAPPING } from "./common";
import {Avatar, Button, Col, Input, Row, Segmented, Select, theme} from "antd";
...
const Swap_box:React.FC<{ }> = ({ }) => {
    ...
    // const [setpool_fee]=useState<string>('');
    ...
    const create_swap = async() =>{
        ...
        try {
            if (swap_to_coin != swap_from_coin) {
                ...
                console.log(`Check on explorer: https://explorer.aptoslabs.com/txn/${response?.hash}?network=testnet`);
    ...
    const ask = async ()=>{
        try {
            ...
            if (swap_from_coin == "APT") {
                ...
                if (swap_to_coin == "USDT") {
                    let usdtRate = await calculateRate("",  "0xf22bede237a07e121b56d91a491eb7bcdfd1f5907926a9e58338f964a01b17fa::asset::USDT",parseFloat(amount)*1000000, 'Uncorrelated', "from", 0.5);
                    let b= parseFloat(usdtRate)/1000000;
                    setrate(b.toString());
                }
                if(swap_to_coin == "APT"){
                    let usdtRate = await calculateRate("", "", parseFloat(amount)*1000000, 'Uncorrelated', "from", 0.5);
